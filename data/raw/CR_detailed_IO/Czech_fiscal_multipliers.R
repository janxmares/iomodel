...
setwd("C:/Rstudio/CR_detailed_IO/")
file_name_total <- "data/SIOT_2015s_CZ__CPA184_pxp.xls"
file_name_dom <- "data/SIOT_DOM_2015s_CZ__CPA184_pxp.xls"
multiplier_output_file <- "results/multipliers_table.csv"
fiscal_stimulus_file <- "data/fiscal_stimulus.xls"
fiscal_stimulus_effects_file <- "results/stimulus_effects_table.csv"

# Reading data, total table and preparing the matrices
year <- read_excel(file_name_total, range = "A3:A3",col_names = FALSE)
sector_codes <- read_excel(file_name_total, range = "A9:A192",col_names = FALSE)
...
setnames(Multipliers_table, c("Kód kategorie výrobku","Název kategorie","Output multipliers - DOM","Output multipliers - TOT","VA multipliers - DOM","VA multipliers - TOT","Employment multipliers - DOM","Employment multipliers - TOT"))
...
setnames(Stimulus_effects_table, c("Kód kategorie výrobku","Název kategorie","Stimulus", "Dopad na produkci TI - (mil. Kc), DOM","Dopad na produkci TII - (mil. Kc), DOM","Dopad na pridanou hodnotu TI - - (mil. Kc), DOM","Dopad na pridanou hodnotu TII - - (mil. Kc), DOM","Dopad na pocet pracovních míst TI"))
